...
const sendCredentials = async () => {
  try {
    ...
    for (const facility of facilities) {
      ...
      const pendingCredentialsModel = await getModel(
        "PendingCredential",
        payservedb.PendingCredential.schema,
        facilityId,
      );

      const pendingCredentials = await pendingCredentialsModel
        .find({
          status: "pending",
          retryCount: { $lt: 3 },
        })
      ...
      console.log(
        `Processing ${pendingCredentials.length} pending credentials for facility ${facilityId}`,
      );
      ...
      for (const credential of pendingCredentials) {
        try {
          ...
          console.log(
            `✓ Sent credentials to ${credential.phoneNumber} [${facilityId}]`,
          );
          console.log(
            `✓ Sent credentials to ${credential.email} [${facilityId}]`,
          );
        } catch (error) {
          console.error(
            `× Failed to send credentials to ${credential.email} [${facilityId}]:`,
            error,
          );
...
const startCredentialSender = () => {
  ...
  setTimeout(
    () => {
      if (job) {
        job.stop();
        console.log("Credential sender job stopped after 10 minutes");
      }
    },
    10 * 60 * 1000,
  );
...
//     runCount++;
//     console.log(`Running job #${runCount}`);

//     await sendCredentials();

//     if (runCount >= MAX_RUNS) {
//       job.stop();
//       console.log(`Credential sender job stopped after ${MAX_RUNS} runs`);
//     }
//   });

//   console.log(`Credential sender job scheduled (will run ${MAX_RUNS} times every 2 minutes)`);
// };
