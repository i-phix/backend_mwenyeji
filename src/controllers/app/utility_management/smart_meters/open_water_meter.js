...
const openMeter = async (request, reply) => {
  try {
    ...
    const meterServiceUrl = process.env.sandboxOpenMeterEndUrl;
    ...
    return reply.code(200).send({
      success: true,
      message: 'The meter will be opened shortly',
      data: response.data
    });
  } catch (error) {
    console.error('Error forwarding open meter request:', error);
