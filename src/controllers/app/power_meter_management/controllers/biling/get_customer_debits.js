...
const getAccountDebits = async (request, reply) => {
    try {
        ...
        const response = await axios.get(`${process.env.POWER_BILLING_SERVICE_APP_URL}/debits/${accountId}`);

        // Return the response from the billing service
        return reply.code(200).send(response.data);
    } catch (err) {
        logger.error('Error in getAccountDebits:', {
            error: err.message,
            stack: err.stack,
            params: {
...
module.exports = getAccountDebits;