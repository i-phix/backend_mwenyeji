...
const validateGLAccounts = async (glAccounts, facilityId, glAccountModel) => {
  // COMMENTED OUT: All GL account validation restrictions
  // if (!glAccounts) {
  ...
  // If no GL accounts provided, skip validation
  if (!glAccounts || !glAccounts.invoice?.debit || !glAccounts.invoice?.credit ||
      !glAccounts.payment?.debit || !glAccounts.payment?.credit) {
    console.log('GL accounts not provided or incomplete, skipping validation');
    return true;
  }
...
const addLevy = async (request, reply) => {
  try {
    ...
    // COMMENTED OUT: GL accounts validation is now optional
    ...
    // GL accounts validation - now optional, only validate if provided
    if (glAccounts && glAccounts.invoice?.debit && glAccounts.invoice?.credit &&
        glAccounts.payment?.debit && glAccounts.payment?.credit) {
      try {
        await validateGLAccounts(glAccounts, facilityId, glAccountModel);
    ...
    const levyData = {
      ...
      glAccounts: glAccounts && glAccounts.invoice?.debit && glAccounts.invoice?.credit &&
                 glAccounts.payment?.debit && glAccounts.payment?.credit ? {
        invoice: {
